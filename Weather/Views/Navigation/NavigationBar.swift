...
struct NavigationBar: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            HStack {
                // MARK: Back Button - кнопка «Назад»
                Button {
                    dismiss()
                } label: {
                    HStack(spacing: 5) {
                        // MARK: Back Button Icon - значок кнопки «Назад»
                        Image(systemName: "chevron.left")
                            .font(.system(size: 23).weight(.medium))
                        .foregroundColor(.secondary)
                       
                        // MARK: Back Button Label - Метка кнопки «Назад»
                        Text("Weather")
                            .font(.title)
                ...
                // MARK: More Button - Кнопка «Дополнительно»
